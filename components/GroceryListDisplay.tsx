...
export const GroceryListDisplay: React.FC<GroceryListDisplayProps> = ({ groceryList, onGenerateGroceryList, isLoading, hasNutritionPlan }) => {
  ...
  if (!hasNutritionPlan) {
     return (
      <div className="text-center p-8 bg-slate-800 rounded-lg shadow-xl">
        <ListIcon className="w-16 h-16 text-emerald-500 mx-auto mb-4" />
        <h2 className="text-2xl font-semibold text-emerald-400 mb-2">No Nutrition Plan Found</h2>
  ...
  if (!groceryList) {
    return (
      <div className="text-center p-8 bg-slate-800 rounded-lg shadow-xl">
        <ListIcon className="w-16 h-16 text-emerald-500 mx-auto mb-4" />
        <h2 className="text-2xl font-semibold text-emerald-400 mb-2">Grocery List</h2>
  ...
  return (
    <div className="space-y-6 p-4 md:p-6 bg-slate-800 rounded-xl shadow-xl">
      <div className="text-center mb-8">
        <ListIcon className="w-12 h-12 text-emerald-500 mx-auto mb-3" />
        <h1 className="text-3xl font-bold text-emerald-400 tracking-tight">Your Grocery List</h1>
         <button
          onClick={onGenerateGroceryList}
      ...
      <div className="space-y-6">
        {groceryList.groceryList.map((category: GroceryCategory, index: number) => (
          <div key={index} className="bg-slate-700 p-4 rounded-lg shadow">
            <h2 className="text-xl font-semibold text-emerald-300 mb-3 border-b border-slate-600 pb-2">{category.category}</h2>
