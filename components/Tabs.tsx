...
const TabButton: React.FC<TabButtonProps> = ({ label, IconComponent, isActive, onClick, hasContent }) => (
  <button
    ...
    className={`
      ...
      focus:outline-none focus:ring-2 focus:ring-sky-500 transition-all duration-150 ease-in-out
    ...
    `}
  >
    <IconComponent className={`w-5 h-5 ${isActive ? 'text-white' : 'text-slate-400 group-hover:text-sky-300'}`} />
